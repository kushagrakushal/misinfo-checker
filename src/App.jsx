...
const BotMessage = ({ message, renderMarkdown }) => {
    ...
    return (
        <div className="max-w-xl p-4 rounded-xl shadow-xl bg-surface text-text-main">
            <div dangerouslySetInnerHTML={renderMarkdown(animatedText)} className="prose prose-sm md:prose-base prose-invert prose-p:text-text-main prose-strong:text-white prose-h2:text-primary" />
            {/* Show sources only after the animation is complete for a cleaner look */}
            {animatedText.length === message.text.length && message.sources && message.sources.length > 0 && (
...
// Updated suggestion card with hover effects
            ...
            <main className="flex-1 overflow-y-auto p-4 md:p-6 flex flex-col">
                {messages.length <= 1 ? (
                    <div className="flex-1 flex flex-col items-center justify-center text-center animate-fade-in-up">
                        <div className="p-6 bg-surface/50 rounded-full shadow-2xl mb-6 border border-secondary">
                        ...
                        <p className="text-lg text-text-secondary mb-8 max-w-2xl">
                            Paste a news snippet, an article, or any text you're unsure about. I'll analyze it for bias, emotional language, and logical fallacies.
                        </p>
                    ...
                    <div className="max-w-3xl mx-auto space-y-6 w-full">
                        {messages.map((msg, index) => (
                            <div key={index} className={`flex items-start gap-4 ${msg.isUser ? 'justify-end' : 'justify-start'} animate-fade-in-up`}>
                                ...
                                {msg.isUser ? (
                                    <div className="max-w-xl p-4 rounded-xl shadow-lg bg-gradient-to-br from-primary to-primary-hover text-white font-semibold">
                                        <p>{msg.text}</p>
                                    </div>
                        ...
                        {isLoading && (
                             <div className="flex items-start gap-4 justify-start animate-fade-in-up">
                                <div className="flex-shrink-0 w-10 h-10 rounded-full bg-secondary flex items-center justify-center"><Bot className="text-primary"/></div>
                                <div className="max-w-xl p-4 rounded-xl shadow-md bg-surface text-text-main">
                                    <div className="flex items-center space-x-2 text-text-secondary"><Loader className="animate-spin" size={20} /><span>Analyzing...</span></div>
                                </div>
                            </div>
                        )}
                         {error && (
                             <div className="flex items-start gap-4 justify-start animate-fade-in-up">
                                 <div className="flex-shrink-0 w-10 h-10 rounded-full bg-red-500/50 flex items-center justify-center"><AlertTriangle className="text-red-300" /></div>
                ...
                <div className="max-w-3xl mx-auto">
                    <form onSubmit={handleSubmit} className="flex items-center bg-surface rounded-lg p-2 ring-1 ring-secondary focus-within:ring-2 focus-within:ring-primary transition-shadow">
                        <textarea
                            ...
                            disabled={isLoading}
                        />
                        <button type="submit" className="bg-primary text-background rounded-md p-2 ml-2 hover:bg-primary-hover focus:outline-none focus:ring-2 focus:ring-primary/50 disabled:bg-secondary disabled:cursor-not-allowed transition-colors" disabled={isLoading || !input.trim()}>
